#!/usr/bin/env python3
"""
Comprehensive test suite for the GNews MCP server.
...
async def run_tool_test(
    ...
    try:
        ...
        from gnews_mcp_server.server import TOOL_FUNCTIONS
        ...
        if test_case.get("should_succeed", True):
            ...
            with open("gnews_mcp_server/tools.json", "r") as f:
...
async def test_server_components():
    ...
    try:
        ...
        from gnews_mcp_server.server import load_tool_schemas, TOOL_FUNCTIONS
...
async def run_tool_tests(schemas: Dict[str, Any]):
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        # Add delay between tests that likely make API calls to avoid rate limiting
        if i > 1 and test_case.get("should_succeed", True):
            time.sleep(1.5)  # 1.5 second delay between successful API tests
